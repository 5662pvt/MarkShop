    ...
    public class UserController : Controller
    {
        ...
        public ActionResult DangKy(Information user)
        {
            try
            {
                ...
                KhachHang checkKH = db.KhachHangs.SingleOrDefault(n => n.TaiKhoan.Equals(user.tendangnhap)); //trả về 1 item duy nhất phù hợp với điều kiện, nếu không có Item nào thỏa mãn, default value sẽ được trả về. 
                if (checkKH != null)
                {
        ...
        [HttpPost]
        public ActionResult DangNhap(Information user, FormCollection f)     //FormCollection: truyền dữ liệu từ view sang controller
        {
            if (!String.IsNullOrEmpty(user.UserName) && !String.IsNullOrEmpty(user.Password))
            {
                ...
                else if ("AD".Equals(user.UserName) && "123".Equals(user.Password))
                {
        ...
        {
            ViewBag.loaiSP = db.LoaiSanPhams.OrderBy(sp => sp.MaLoaiSP);         //viewbag: truyền dữ liệu từ controller sang view
